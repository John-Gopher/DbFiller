...
PHP_METHOD(dbfiller, connect) {
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "s", &arg, &arg_len) == FAILURE) {
        ...
        if (mysql_real_connect(&conn,  Z_STRVAL_P(host),Z_STRVAL_P(user), Z_STRVAL_P(pwd),arg,Z_STRVAL_P(port), NULL, 128) == NULL)
	{
		php_printf("Connection fails.\n");
		mysql_close(&conn);
		return;
	}
        //ZEND_REGISTER_RESOURCE(return_value,&conn,le_sample_descriptor);
        temp_conn = &conn;
       // ZEND_FETCH_RESOURCE(temp_conn,MYSQL*,&return_value,-1,PHP_MY_RES_NAME,le_sample_descriptor);
        char sql[] = "show tables";
        MYSQL_RES *mysql_result;
       	if( mysql_real_query(temp_conn,sql,strlen(sql)) != 0){
		php_printf("mysql query error!\n");
	}
        mysql_result = mysql_store_result(temp_conn);
	...
	for(i = 0; i < j; i++)       
	{
        ...
        printf("%d\n",mysql_result->row_count);
        */
         ZEND_REGISTER_RESOURCE(return_value,mysql_result,le_result_des);
...
PHP_METHOD(dbfiller,build) {
	...
	char sql[] = "show tables";
        MYSQL_RES *mysql_result;
//	MYSQL_ROW  row;
        //zv_conn  = zend_read_property(dbfiller_ce, getThis(), "conn", sizeof("conn")-1, 0 TSRMLS_DC);

        //ZEND_FETCH_RESOURCE(conn,MYSQL*,&zv_conn,-1,PHP_MY_RES_NAME,le_sample_descriptor);
	...
	//exit(1);
        //ZEND_FETCH_RESOURCE(conn,MYSQL*,arg,-1,PHP_MY_RES_NAME,le_sample_descriptor);
        ZEND_FETCH_RESOURCE(mysql_result,MYSQL_RES*,arg,-1,PHP_MY_RES_NAME,le_result_des);
         //mysql_real_query(conn,sql,strlen(sql));
        printf("build:%d\n",mysql_result->row_count);
